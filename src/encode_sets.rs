...
pub const QUERY_PERCENT_ENCODE_SET: &AsciiSet = &CONTROLS.add(b' ').add(b'"').add(b'#').add(b'<').add(b'>');

/// The path percent-encode set is the query percent-encode set and U+003F (?), U+0060 (`), U+007B ({), and U+007D (}).
pub const PATH_PERCENT_ENCODE_SET: &AsciiSet = &CONTROLS.add(b'?').add(b'`').add(b'{').add(b'}');
...
pub const USER_INFO_PERCENT_ENCODE_SET: &AsciiSet = &CONTROLS.add(b'/').add(b':').add(b';').add(b'=').add(b'@').add(b'[').add(b']').add(b'^').add(b'|');

/// The fragment percent-encode set is the C0 control percent-encode set and U+0020 SPACE, U+0022 ("), U+003C (<), U+003E (>), and U+0060 (`).
pub const FRAGMENT_PERCENT_ENCODE_SET: &AsciiSet = &CONTROLS.add(b' ').add(b'"').add(b'<').add(b'>').add(b'`');
